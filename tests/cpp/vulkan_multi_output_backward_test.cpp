...
void test_output_overlap_rejected(VulkanPlatform &platform) {
    ...
    VulkanTensorLayout db{1, {2}, {1}, 0,  sizeof(float),    0,
                          2, 0,   8,   64, VulkanOverlap::No};
    dw.byte_offset = 4;
    ...
    try {
        ...
        rejected_overlap =
            std::string(error.what()).find("outputs overlap") != std::string::npos;
    }
    if (!rejected_overlap)
        throw std::runtime_error(
            "overlap fixture did not reach output-overlap validation: " +
            overlap_error);
}
