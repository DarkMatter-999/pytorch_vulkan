...
def test_migrated_linear_backward_accepts_a_view(vulkan_backend):
    ...
    torch.testing.assert_close(
        vk_input.grad.cpu(), torch.ones_like(input_view) @ weight_view
    )
    torch.testing.assert_close(
        vk_weight.grad.cpu(), torch.ones_like(output.cpu()).t() @ input_view
    )
...
def test_vulkan_activation_backward_matches_cpu(
    vulkan_backend, operation, cpu_operation
):
    cpu = torch.tensor([-2.0, -0.25, 0.0, 0.5, 3.0], requires_grad=True)
    vk = cpu.detach().clone().to(vulkan_backend).requires_grad_()
    ...
    assert vk.grad.device == vk.device
    torch.testing.assert_close(
        vk_result.cpu(), cpu_result.detach(), rtol=2e-5, atol=2e-6
    )
...
def test_vulkan_activation_higher_order_backward_is_rejected(vulkan_backend, operation):
    value = torch.tensor(
        [-1.0, 0.5], dtype=torch.float32, device=vulkan_backend, requires_grad=True
    )
