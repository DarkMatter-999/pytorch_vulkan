...
def test_native_batch_norm_fixed_training_contract_matches_cpu(vulkan_backend):
    ...
    expected = torch.ops.aten.native_batch_norm.default(
        *cpu, True, 0.1, 1e-5
    )
    actual = torch.ops.aten.native_batch_norm.default(
        *vk, True, 0.1, 1e-5
    )
...
def test_native_batch_norm_rejects_eval_and_wrong_eps_without_vulkan_work(vulkan_backend):
...
def test_native_batch_norm_backward_returns_resident_gradients(vulkan_backend):
    ...
    vk = tuple(value.to(vulkan_backend).requires_grad_(value.requires_grad) for value in cpu)
    ...
    actual = torch.ops.aten.native_batch_norm_backward.default(
        grad.to(vulkan_backend), vk[0], vk[1], vk[3], vk[4], vk_forward[1],
        vk_forward[2], True, 1e-5, [True, True, True]
    )
    for result, reference in zip(actual, expected):
        assert result.device == torch.device(vulkan_backend)
        torch.testing.assert_close(result.cpu(), reference)
