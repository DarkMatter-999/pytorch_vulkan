...
def tensors(device, requires_grad=False):
    x = torch.tensor(
        [[1.0, -2.0], [3.0, 4.0]], device=device, requires_grad=requires_grad
    )
    y = torch.tensor([[0.5, 1.0], [2.0, 5.0]], device=device)
    return x, y
...
def test_mse_forward_backward_parity_and_residency(vk, reduction):
    ...
    expected = torch.nn.functional.mse_loss(
        x.cpu().detach(), y.cpu(), reduction=reduction
    )
...
@pytest.mark.parametrize(
    "case", ["shape", "reduction", "target_dtype", "target_device", "target_layout"]
)
