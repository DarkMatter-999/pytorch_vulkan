...
def test_all(device):
    print("Mean 1d")
    test_fwd_bwd(
        [([2, 3, 4], -1)], lambda x: torch.mean(x, dim=0, keepdim=True), device
    )
    print("Mean 2d")
    test_fwd_bwd(
        [([2, 3, 4], -1)], lambda x: torch.mean(x, dim=(1, 2), keepdim=True), device
    )
    print("Mean 1d squeeze")
    test_fwd_bwd(
        [([2, 3, 4], -1)], lambda x: torch.mean(x, dim=0, keepdim=False), device
    )
    print("Mean 2d squeeze")
    test_fwd_bwd(
        [([2, 3, 4], -1)], lambda x: torch.mean(x, dim=(0, 2), keepdim=False), device
    )
    ...
    print("Sum 2d")
    test_fwd_bwd(
        [([2, 3, 4], -1)], lambda x: torch.sum(x, dim=(1, 2), keepdim=True), device
    )
    print("Sum 1d squeeze")
    test_fwd_bwd(
        [([2, 3, 4], -1)], lambda x: torch.sum(x, dim=0, keepdim=False), device
    )
    print("Sum 2d squeeze")
    test_fwd_bwd(
        [([2, 3, 4], -1)], lambda x: torch.sum(x, dim=(0, 2), keepdim=False), device
    )

    print("Prod 1d")
    test_fwd_bwd(
        [([2, 3, 4], -1)], lambda x: torch.prod(x, dim=1, keepdim=True), device
    )
    print("Prod 1d squeeze")
    test_fwd_bwd(
        [([2, 3, 4], -1)], lambda x: torch.prod(x, dim=1, keepdim=False), device
    )
    ...
    print("Hardtanh_")
    test_fwd_bwd(
        [([4, 3], -1)], lambda x: torch.nn.Hardtanh(inplace=True)(x * 1.0), device
    )
    print("Sigmoid")
    ...
    print("Hardsigmoid_")
    test_fwd_bwd(
        [([4, 3], -1)], lambda x: torch.nn.Hardsigmoid(inplace=True)(x * 1.0), device
    )
    print("ReLU")
    ...
    print("LReLU_")
    test_fwd_bwd(
        [([4, 3], -1)], lambda x: torch.nn.LeakyReLU(inplace=True)(x * 1.0), device
    )
    print("Tanh")
    ...
    print("GELU tanh")
    test_fwd_bwd(
        [([4, 3], -1)], lambda x: torch.nn.GELU(approximate="tanh")(x * 1.0), device
    )
    ...
    print("BCE Loss")
    test_fwd_bwd(
        [([4, 3, 5], -1), ([4, 3, 5], -1)], torch.nn.BCELoss(), device, torch.rand
    )
    ...
    print("MSE Loss")
    test_fwd_bwd(
        [([4, 3, 5], -1), ([4, 3, 5], -1)], torch.nn.MSELoss(), device, torch.rand
    )
    ...
    print("Lerp")
    test_fwd(
        [([4, 3, 5], -1), ([4, 3, 1], -1)], lambda x, y: torch.lerp(x, y, 0.1), device
    )
    print("Dot")
    ...
    test_fwd_bwd_op(
        [([2, 6, 10, 20], -1)],
        torch.nn.Conv2d(
            6, 8, [3, 5], stride=[1, 2], padding=[1, 2], dilation=1, groups=2
        ),
        device,
    )
    print("ConvTr")
    ...
    test_fwd_bwd_op(
        [([2, 3, 4, 30], -1)],
        torch.nn.LayerNorm((4, 30), elementwise_affine=False),
        device,
    )
    ...
    test_fwd_bwd_op(
        [([2, 3, 4, 30], -1)],
        torch.nn.LayerNorm((4, 30), elementwise_affine=True),
        device,
        paramgen=torch.randn,
    )
    ...
    test_fwd(
        [([4, 3, 5], -1)],
        lambda x: torch.logit(torch.clamp(x, min=0.1, max=0.9)),
        device,
    )
    ...
    test_fwd_bwd(
        [([2, 3, 4, 5], -1)], lambda x: torch.amax(x, (1, -1), keepdim=True), device
    )
    ...
    test_fwd_bwd(
        [([2, 3, 4, 5], -1)], lambda x: torch.amin(x, (1, -1), keepdim=True), device
    )
    ...
    test_fwd(
        [([2, 4], -1), ([2, 3], -1), ([3, 4], -1)],
        lambda a, b, c: torch.addmm(a, b, c, beta=0.0, alpha=1.0),
        device,
    )
    print("Test addmm 1/1")
    test_fwd(
        [([2, 4], -1), ([2, 3], -1), ([3, 4], -1)],
        lambda a, b, c: torch.addmm(a, b, c, beta=1.0, alpha=1.0),
        device,
    )
    print("Test addmm 1.0/0.5")
    test_fwd(
        [([2, 4], -1), ([2, 3], -1), ([3, 4], -1)],
        lambda a, b, c: torch.addmm(a, b, c, beta=1.0, alpha=0.5),
        device,
    )
    print("Test addmm 0.5/0.5")
    test_fwd(
        [([2, 4], -1), ([2, 3], -1), ([3, 4], -1)],
        lambda a, b, c: torch.addmm(a, b, c, beta=0.5, alpha=0.5),
        device,
    )

    print("Test addmm 0/1 cp")
    test_fwd(
        [([2, 4], -1), ([2, 3], -1), ([3, 4], -1), ([4, 2], -1)],
        lambda a, b, c, o: torch.addmm(a, b, c, beta=0.0, alpha=1.0, out=o.T),
        device,
    )
    print("Test addmm 1/1 cp")
    test_fwd(
        [([2, 4], -1), ([2, 3], -1), ([3, 4], -1), ([4, 2], -1)],
        lambda a, b, c, o: torch.addmm(a, b, c, beta=1.0, alpha=1.0, out=o.T),
        device,
    )
    print("Test addmm 1.0/0.5 cp")
    test_fwd(
        [([2, 4], -1), ([2, 3], -1), ([3, 4], -1), ([4, 2], -1)],
        lambda a, b, c, o: torch.addmm(a, b, c, beta=1.0, alpha=0.5, out=o.T),
        device,
    )
    print("Test addmm 0.5/0.5 cp")
    test_fwd(
        [([2, 4], -1), ([2, 3], -1), ([3, 4], -1), ([4, 2], -1)],
        lambda a, b, c, o: torch.addmm(a, b, c, beta=0.5, alpha=0.5, out=o.T),
        device,
    )

    print("Test _transform_bias_rescale_qkv")
    test_fwd(
        [([5, 7, 3 * 2 * 11], -1), ([3 * 2 * 11], -1)],
        lambda a, b: torch.cat(torch._transform_bias_rescale_qkv(a, b, 2), dim=0),
        device,
    )
    ...
    test_fwd_bwd(
        ...
        lambda x: torch.nn.functional.interpolate(
            x, scale_factor=(2.3, 1.5), mode="nearest"
        ),
        device,
    )
    ...
    test_fwd_bwd(
        [([2, 3, 10, 15], -1)],
        lambda x: torch.nn.functional.interpolate(x, size=(20, 20), mode="nearest"),
        device,
    )
    ...
    test_fwd_bwd(
        [([2, 3, 10, 15], -1)],
        lambda x: torch.nn.functional.interpolate(
            x, scale_factor=(2.3, 1.5), mode="nearest-exact"
        ),
        device,
    )
    ...
    test_fwd_bwd(
        [([2, 3, 10, 15], -1)],
        lambda x: torch.nn.functional.interpolate(
            x, size=(20, 20), mode="nearest-exact"
        ),
        device,
    )
    ...
    test_fwd_bwd(
        [([2, 3, 10, 15], -1)],
        lambda x: torch.nn.functional.interpolate(
            x, scale_factor=(2.3, 1.5), mode="bilinear"
        ),
        device,
    )
    ...
    test_fwd_bwd(
        [([2, 3, 10, 15], -1)],
        lambda x: torch.nn.functional.interpolate(x, size=(20, 20), mode="bilinear"),
        device,
    )
    ...
    test_fwd_bwd(
        [([2, 3, 10, 15], -1)],
        lambda x: torch.nn.functional.interpolate(
            x, scale_factor=(2.3, 1.5), mode="bilinear", align_corners=True
        ),
        device,
    )
    ...
    test_fwd_bwd(
        [([2, 3, 10, 15], -1)],
        lambda x: torch.nn.functional.interpolate(
            x, size=(20, 20), mode="bilinear", align_corners=True
        ),
        device,
    )
...
def test_logical(device):
    ...
    for with_bool, cases in [(True, l_bool_int)]:
        for name, op in cases:
            print("Test ", name)
            test_fwd(
                [([20, 30], 10), ([1, 30], 10)], lambda a, b: op(a - 5, a - 5), device
            )
            if with_bool:
                print("Test ", name, " with bool")
                test_fwd(
                    [([20, 30], 10), ([1, 30], 10)],
                    lambda a, b: op(a > 5, a > 5).to(torch.float32),
                    device,
                )
...
def test_comp(device):
    ...
    for with_scal, cases in [(True, l2), (False, l1)]:
        for name, op in cases:
            ...
            test_fwd(
                [([20, 30], -1), ([1, 30], -1)],
                lambda a, b: op(torch.round(a) * 10, torch.round(b) * 10).to(
                    torch.float
                ),
                device,
            )
            if with_scal:
                ...
                test_fwd(
                    [([20, 30], -1)],
                    lambda a: op(torch.round(a) * 10, 5).to(torch.float),
                    device,
                )
            ...
            test_fwd(
                [([20, 30], -1)],
                lambda a: op(
                    torch.round(a) * 10, torch.tensor(5, dtype=torch.float32)
                ).to(torch.float),
                device,
            )
            ...
            test_fwd(
                [([20, 30], -1)],
                lambda a: op(
                    torch.tensor(5, dtype=torch.float32), torch.round(a) * 10
                ).to(torch.float),
                device,
            )
    for name, op in [
