...
def make_batch(size):
    ...
    dirname = {"standard": "samples", "small": "small_samples", "large": "big_samples"}[
        size
    ]
...
def main(args):
    ...
    if args.model in models:
        ...
        if args.model == "sn":
            ...
            m = getattr(
                torchvision.models.segmentation, args.model[len("segmentation.") :]
            )(aux_loss=False, weights=weights)
        else:
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--iter-size", default=1, type=int, help="Number of mini batches in iteration"
    )
    ...
    if r.all:
        ...
        for net in [
            ...
            dict(
                model="mobilenet_v3_small", eval=True, fwd=True
            ),  # fails for cuda bwd as well
            ...
            if net["model"] in ocl_blacklist and (
                r.device.find("ocl") == 0 or r.device.find("private") == 0
            ):
