...
def assert_contracts():
    ...
    bindings = re.findall(r"layout\(set = 0, binding = (\d+), std430\) (\w+)", normalization)
    assert [(int(binding), access) for binding, access in bindings] == [(i, "buffer") for i in range(10)]
    ...
    assert "uint mode; uint batch; uint channels; uint spatial; uint classes;" in normalization
    assert "uint ignore_index; uint padding0; uint padding1; float momentum; float eps;" in normalization
