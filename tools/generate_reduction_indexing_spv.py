...
with OUT.open("w", encoding="ascii") as header:
    ...
    for name, shader in SHADERS:
        ...
        words = [int.from_bytes(binary.read_bytes()[i:i + 4], "little") for i in range(0, binary.stat().st_size, 4)]
...
spirv = b"".join((pathlib.Path("/tmp") / (shader + ".spv")).read_bytes()
                   for _, shader in SHADERS)
def digest(data):
    return hashlib.sha256(data).hexdigest()
MANIFEST.write_text(
    "reduction_source_sha256=" + digest(b"".join((OUT.parent.parent / "glsl" / shader).read_bytes() for _, shader in SHADERS)) + "\n"
