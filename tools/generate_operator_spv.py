...
with HEADER.open("w", encoding="ascii") as header:
    ...
    for name, code in words:
        namespace = (
            "vulkan_normalization_shader"
            if name == "normalization"
            else "vulkan_classification_shader"
        )
        header.write(
            f"namespace {namespace} {{ inline constexpr uint32_t kCode[] = {{\n"
        )
        for i in range(0, len(code), 8):
            header.write(
                "    " + ", ".join(f"0x{x:08x}U" for x in code[i : i + 8]) + ",\n"
            )
