...
def main():
    expected = dict(line.split("=", 1) for line in MANIFEST.read_text(encoding="ascii").splitlines() if "=" in line)
    ...
    with tempfile.TemporaryDirectory() as directory:
        binaries = []
        for name, define in (("kCountCode", "MASKED_SELECT_COUNT"), ("kCompactCode", None)):
            path = pathlib.Path(directory) / f"{name}.spv"
            command = ["glslc", "-Os"] + ([f"-D{define}"] if define else []) + ["-o", str(path), str(SOURCE)]
    ...
    actual = {"source_sha256": sha256(SOURCE.read_bytes()), "spirv_sha256": sha256(b"".join(binaries)), "header_sha256": sha256(OUTPUT.read_bytes())}
