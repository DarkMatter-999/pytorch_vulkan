...
dlprim::core::Conv2DSettings conv_config(bool transposed, dlprim::Tensor &X,
                                         dlprim::Tensor &W, IntArrayRef padding,
                                         IntArrayRef stride, IntArrayRef dilation,
                                         IntArrayRef output_padding, int groups) {
    ...
    if (!transposed) {
        ...
        return dlprim::core::Conv2DSettings(
            cfg_base,
            dlprim::core::Conv2DBase::get_output_shape_transposed(cfg_base, X.shape(),
                                                                  op),
            X.dtype());
    }
}
...
Tensor convolution_overrideable(const Tensor &input, const Tensor &weight,
    ...
    dlprim::core::Conv2DSettings cfg = conv_config(transposed, X, W, padding, stride,
    ...
    if (!transposed) {
        ...
        dlprim::Shape rs =
            dlprim::core::Conv2DForward::get_output_shape(cfg, X.shape());
...
// {"schema": "aten::convolution_backward_overrideable(Tensor grad_output, Tensor input,
// Tensor weight, int[] stride, int[] padding, int[] dilation, bool transposed, int[]
// output_padding, int groups, bool[3] output_mask) -> (Tensor grad_input, Tensor
// grad_weight, Tensor grad_bias)", "dispatch": "True", "default": "True"}
::std::tuple<Tensor, Tensor, Tensor> convolution_backward_overrideable(
    ...
    if (output_mask[0]) {
        if (!transposed) {
            ...
            bwd_data_tr =
                std::move(dlprim::core::Conv2DForward::create(ctx, cfg, false));
    ...
    if (output_mask[2]) {
        bwd_bias = std::move(
            dlprim::core::BiasBackwardFilter::create(ctx, dy.shape(), dy.dtype()));
    ...
    return std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>(
        data_diff, filter_diff, bias_diff);
}

Tensor
_adaptive_avg_pool2d(const Tensor &self,
                     IntArrayRef output_size) // {"schema": "aten::_adaptive_avg_pool2d
{
    GUARD;
...
// {"schema": "aten::_adaptive_avg_pool2d_backward(Tensor grad_output, Tensor self) ->
...
Tensor _adaptive_avg_pool2d_backward(const Tensor &grad_output, const Tensor &self) {
    ...
    dlprim::Tensor dy = todp(grad_output_c);
    torch::Tensor result;
    TORCH_CHECK((dy.shape()[2] == 1 && dy.shape()[3] == 1) || (dy.shape() == X.shape()),
                "Only global pooling or no-pooling supported");
    if (dy.shape()[2] == 1 && dy.shape()[3] == 1) {
        ...
        dlprim::ExecutionContext q = getExecutionContext(self);
        dlprim::Context ctx(q);
        auto pool = dlprim::core::AvgPooling2DBackward::create_global(
            ctx, X.shape(), todp(self.dtype()));
...
class linear_cls : public torch::autograd::Function<linear_cls> {
  public:
    static Tensor forward(AutogradContext *ctx, const Tensor &input,
                          const Tensor &weight, const c10::optional<Tensor> &bias) {
        return linear_forward(ctx, input, weight, bias);
    }
    static Tensor linear_forward(AutogradContext *ctx, const Tensor &input,
                                 const Tensor &weight,
    ...
    static tensor_list linear_backward(AutogradContext *ctx, tensor_list grad_outputs) {
        ...
        if (has_bias) {
            ...
            auto bwd_bias = dlprim::core::BiasBackwardFilter::create(
                dlprim_ctx, dY.shape(), cfg.dtype);
            at::DataPtr ptr;
            dlprim::Tensor ws =
                make_workspace(ptr, bwd_bias->workspace(), dy_tensor.device());
            bwd_bias->enqueue(dY, dB, ws, 0, q);
        }
...
class max_pool2d_cls : public torch::autograd::Function<max_pool2d_cls> {
  public:
    static torch::Tensor forward(AutogradContext *ctx, torch::Tensor const &self,
                                 IntArrayRef kernel_size, IntArrayRef stride,
                                 IntArrayRef padding, IntArrayRef dilation,
                                 bool ceil_mode) {
        return max_pool2d_forward(ctx, self, kernel_size, stride, padding, dilation,
                                  ceil_mode);
    }

    static torch::Tensor max_pool2d_forward(AutogradContext *ctx,
                                            torch::Tensor const &self,
                                            IntArrayRef kernel_size, IntArrayRef stride,
                                            IntArrayRef padding, IntArrayRef dilation,
                                            bool ceil_mode) {
        GUARD;
        ...
        TORCH_CHECK(dilation[0] == 1 && dilation[1] == 1,
                    "Dilation != 1 is not implemented yet");
        ...
        dlprim::Shape y_shape =
            dlprim::Shape(x_shape[0], x_shape[1],
    ...
    static tensor_list max_pool_2d_backward(AutogradContext *ctx,
...
// int[2] padding=0, int[2] dilation=1, bool ceil_mode=False) -> Tensor", "dispatch":
// "False", "default": "True"}
torch::Tensor max_pool2d_autograd(torch::Tensor const &self, IntArrayRef kernel_size,
                                  IntArrayRef stride, IntArrayRef padding,
                                  IntArrayRef dilation, bool ceil_mode) {
    GUARD;
    return max_pool2d_cls::apply(self, kernel_size, stride, padding, dilation,
                                 ceil_mode);
}

// {"schema": "aten::avg_pool2d.out(Tensor self, int[2] kernel_size, int[2] stride=[],
// int[2] padding=0, bool ceil_mode=False, bool count_include_pad=True, int?
// divisor_override=None, *, Tensor(a!) out) -> Tensor(a!)", "dispatch": "True",
// "default": "False"}
Tensor &avg_pool2d_out(const Tensor &self, IntArrayRef kernel_size, IntArrayRef stride,
                       IntArrayRef padding, bool /*ceil_mode*/, bool count_include_pad,
                       c10::optional<int64_t> divisor_override, Tensor &out) {
    GUARD;
...
// {"schema": "aten::avg_pool2d_backward.grad_input(Tensor grad_output, Tensor self,
// int[2] kernel_size, int[2] stride, int[2] padding, bool ceil_mode, bool
// count_include_pad, int? divisor_override, *, Tensor(a!) grad_input) -> Tensor(a!)",
// "dispatch": "True", "default": "False"}
Tensor &avg_pool2d_backward_out(const Tensor &grad_output, const Tensor &self,
                                IntArrayRef kernel_size, IntArrayRef stride,
                                IntArrayRef padding, bool ceil_mode,
...
static Tensor get_bmm_mm_valid_tensor(Tensor const &t, bool &transposed, int &ld,
                                      bool &copied, int bmm, char /*M*/) {
    auto sizes = t.sizes();
...
Tensor &mm_bmm_out(const Tensor &self, const Tensor &mat2, Tensor &out, int bmm) {
    ...
    TORCH_CHECK(M == Mc && N == Nc && Ka == Kb,
                "Invalid matrix sizes "
                "A(" +
                    std::to_string(M) + "," + std::to_string(Ka) + ")" +
                    (At ? ".T" : "  ") + "*B(" + std::to_string(Kb) + "," +
                    std::to_string(N) + ")=" + (Bt ? ".T" : "  ") + "C(" +
                    std::to_string(Mc) + "," + std::to_string(Nc) + ")");

    TORCH_CHECK(A.dtype() == B.dtype() && A.dtype() == C.dtype(),
                "All matrices must have same dtype");
    if (bmm) {
        TORCH_CHECK(A.sizes()[0] == B.sizes()[0] && A.sizes()[0] == C.sizes()[0],
    ...
    if (bmm == 0) {
        auto gemm_op =
            dlprim::gpu::GEMM::get_optimal_gemm(ctx, todp(A.dtype()), At, Bt, M, N, K);
        gemm_op->gemm(M, N, K, Abuf, Aoff, lda, Bbuf, Boff, ldb, Cbuf, Coff, ldc,
                      nullptr, 0, 0, M * N, q);
    } else {
        int batch = A.sizes()[0];
        ...
        dlprim::gpu::GEMM::batch_sgemm(todp(A.dtype()), At, Bt, batch, M, N, K, Abuf,
                                       Aoff, step_A, lda, Bbuf, Boff, step_B, ldb, Cbuf,
                                       Coff, step_C, ldc, 0.0f, q);
    }
    if (Cc)
        out.copy_(C);
    sync_if_needed(self.device());
...
// {"schema": "aten::bmm.out(Tensor self, Tensor mat2, *, Tensor(a!) out) ->
...
// {"schema": "aten::addmm.out(Tensor self, Tensor mat1, Tensor mat2, *, Scalar beta=1,
// Scalar alpha=1, Tensor(a!) out) -> Tensor(a!)", "dispatch": "True", "default":
// "False"}
Tensor &addmm_out(const Tensor &self, const Tensor &mat1, const Tensor &mat2,
                  const Scalar &sbeta, const Scalar &salpha, Tensor &out) {
    GUARD;
    ...
    if (beta == 0) {
        ...
        if (out.is_contiguous()) {
            auto y = todp(out);
            dlprim::core::pointwise_operation({x}, {y}, {alpha}, "y0 = x0*w0;",
                                              getExecutionContext(self));
        } else {
            dlprim::core::pointwise_operation({x}, {x}, {alpha}, "y0 = x0*w0;",
                                              getExecutionContext(self));
            out.copy_(p);
        }
    } else {
...
// {"schema": "aten::_native_multi_head_attention.out(Tensor query, Tensor key, Tensor
// value, int embed_dim, int num_head, Tensor qkv_weight, Tensor qkv_bias, Tensor
// proj_weight, Tensor proj_bias, Tensor? mask=None, bool need_weights=True, bool
// average_attn_weights=True, int? mask_type=None, *, Tensor(a!) out0, Tensor(b!) out1)
// -> (Tensor(a!), Tensor(b!))", "dispatch": "True", "default": "True"}
::std::tuple<Tensor &, Tensor &> _native_multi_head_attention_out(
    const Tensor &query, const Tensor &key, const Tensor &value, int64_t embed_dim,
    int64_t num_head, const Tensor &qkv_weight, const Tensor &qkv_bias,
    ...
    ::std::optional<int64_t> mask_type, Tensor &out0, Tensor &out1) {
    GUARD;
    auto r = at::cpu::_native_multi_head_attention(
        query, key, value, embed_dim, num_head, qkv_weight, qkv_bias, proj_weight,
        proj_bias, mask, need_weights, average_attn_weights, mask_type);
    out0.copy_(std::get<0>(r));
    out1.copy_(std::get<1>(r));
    return ::std::tuple<Tensor &, Tensor &>(out0, out1);
}

// {"schema": "aten::_native_multi_head_attention(Tensor query, Tensor key, Tensor
// value, int embed_dim, int num_head, Tensor qkv_weight, Tensor qkv_bias, Tensor
// proj_weight, Tensor proj_bias, Tensor? mask=None, bool need_weights=True, bool
// average_attn_weights=True, int? mask_type=None) -> (Tensor, Tensor)", "dispatch":
// "True", "default": "False"}
...
// {"schema": "aten::_transform_bias_rescale_qkv(Tensor qkv, Tensor qkv_bias, int
// num_heads) -> (Tensor, Tensor, Tensor)", "dispatch": "True", "default": "False"}
// compute q = (q + q_bias) / sqrt(dim_per_head), k = k + k_bias, v = v + v_bias
std::tuple<Tensor, Tensor, Tensor> transform_bias_rescale_qkv(const Tensor &qkv,
    ...
    auto q_k_v_same_order =
        at::empty({B, T, 3, num_head, dim_per_head}, qkv_->options());
    ...
    dlprim::core::pointwise_operation_broadcast(
        {dp_qkv, dp_bias}, {dp_out}, {scale, double(D)},
...
// {"schema": "aten::upsample_nearest2d.out(Tensor self, SymInt[2] output_size, float?
// scales_h=None, float? scales_w=None, *, Tensor(a!) out) -> Tensor(a!)", "dispatch":
// "True", "default": "False"}
Tensor &interpolate_2d_out_internal(const Tensor &self,
...
// {"schema": "aten::upsample_nearest2d_backward.grad_input(Tensor grad_output,
// SymInt[2] output_size, SymInt[4] input_size, float? scales_h=None, float?
// scales_w=None, *, Tensor(a!) grad_input) -> Tensor(a!)", "dispatch": "True",
// "default": "False"}
Tensor &interpolate_2d_backward_out_internal(
    const Tensor &grad_output, c10::SymIntArrayRef /*output_size*/,
    c10::SymIntArrayRef /*input_size*/, ::std::optional<double> scales_h,
    ::std::optional<double> scales_w, Tensor &grad_input,
    dlprim::InterpolateType method, bool align_c = false) {
    GUARD;
    Tensor grad_output_c = grad_output.contiguous();
...
// scales_h=None, float? scales_w=None, *, Tensor(a!) out) -> Tensor(a!)", "dispatch":
// "True", "default": "False"}
Tensor &upsample_nearest2d_out(const Tensor &self, c10::SymIntArrayRef output_size,
                               ::std::optional<double> scales_h,
                               ::std::optional<double> scales_w, Tensor &out) {
    GUARD;
    return interpolate_2d_out_internal(self, output_size, scales_h, scales_w, out,
...
// scales_w=None, *, Tensor(a!) grad_input) -> Tensor(a!)", "dispatch": "True",
// "default": "False"}
Tensor &upsample_nearest2d_backward_out(const Tensor &grad_output,
                                        c10::SymIntArrayRef output_size,
                                        ...
                                        Tensor &grad_input) {
    GUARD;
    return interpolate_2d_backward_out_internal(grad_output, output_size, input_size,
                                                scales_h, scales_w, grad_input,
                                                dlprim::InterpolateType::nearest);
}

// {"schema": "aten::_upsample_nearest_exact2d.out(Tensor self, SymInt[2] output_size,
// float? scales_h=None, float? scales_w=None, *, Tensor(a!) out) -> Tensor(a!)",
// "dispatch": "True", "default": "False"}
Tensor &_upsample_nearest_exact2d_out(const Tensor &self,
                                      c10::SymIntArrayRef output_size,
                                      ::std::optional<double> scales_h,
                                      ::std::optional<double> scales_w, Tensor &out) {
    GUARD;
...
// {"schema": "aten::_upsample_nearest_exact2d_backward.grad_input(Tensor grad_output,
// SymInt[2] output_size, SymInt[4] input_size, float? scales_h=None, float?
// scales_w=None, *, Tensor(a!) grad_input) -> Tensor(a!)", "dispatch": "True",
// "default": "False"}
Tensor &_upsample_nearest_exact2d_backward_out(const Tensor &grad_output,
                                               c10::SymIntArrayRef output_size,
                                               c10::SymIntArrayRef input_size,
                                               ::std::optional<double> scales_h,
                                               ::std::optional<double> scales_w,
...
// {"schema": "aten::upsample_bilinear2d.out(Tensor self, SymInt[2] output_size, bool
// align_corners, float? scales_h=None, float? scales_w=None, *, Tensor(a!) out) ->
// Tensor(a!)", "dispatch": "True", "default": "False"}
Tensor &upsample_bilinear2d_out(const Tensor &self, c10::SymIntArrayRef output_size,
...
// {"schema": "aten::upsample_bilinear2d_backward.grad_input(Tensor grad_output,
// SymInt[2] output_size, SymInt[4] input_size, bool align_corners, float?
// scales_h=None, float? scales_w=None, *, Tensor(a!) grad_input) -> Tensor(a!)",
// "dispatch": "True", "default": "False"}
Tensor &upsample_bilinear2d_backward_out(const Tensor &grad_output,
                                         c10::SymIntArrayRef output_size,
                                         c10::SymIntArrayRef input_size,
                                         bool align_corners,
                                         ::std::optional<double> scales_h,
    ...
    return interpolate_2d_backward_out_internal(
        ...
        dlprim::InterpolateType::bilinear, align_corners);
}
...
TORCH_LIBRARY_IMPL(aten, PrivateUse1, m) {
    ...
    m.impl("aten::_native_multi_head_attention.out",
           &ptdlprim::_native_multi_head_attention_out);
    m.impl("aten::_native_multi_head_attention",
           &ptdlprim::_native_multi_head_attention);
