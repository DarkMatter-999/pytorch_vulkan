...
dlprim::DataType todp(c10::ScalarType tp) {
    ...
    case c10::kBool:
        ...
        return dlprim::uint8_data;
    default:
        throw std::runtime_error(std::string("Unsupported data type:") +
                                 c10::toString(tp));
    }
}

cl::Buffer buffer_from_tensor(torch::Tensor const &tt) {
    TORCH_CHECK(tt.device().type() == OpenCLDeviceType,
                "OpenCL device is required for tensor");
    // TORCH_CHECK(tt.numel() > 0,"Buffer is not valid for unallocated defvice");
    TORCH_CHECK(tt.getIntrusivePtr()->storage().nbytes() > 0,
                "Buffer is not valid for unallocated defvice");
    cl_mem p =
        static_cast<cl_mem>(const_cast<void *>(tt.getIntrusivePtr()->storage().data()));
...
torch::Tensor new_ocl_tensor(torch::IntArrayRef size, c10::Device dev,
                             c10::ScalarType type) {
    size_t n = 1;
    ...
    c10::Storage storage(c10::Storage::use_byte_size_t(), mem,
                         CLContextManager::allocate(dev, mem));
...
torch::Tensor new_tensor_as(dlprim::Shape const &s, torch::Tensor const &as) {
    ...
    torch::Tensor result = new_ocl_tensor(c10::IntArrayRef(shape, s.size()),
                                          as.device(), as.dtype().toScalarType());
    return result;
}

dlprim::Tensor make_workspace(at::DataPtr &ws_ptr, size_t ws_size,
                              c10::Device const &dev) {
    dlprim::Tensor ws;
    if (ws_size) {
        ws_ptr = std::move(CLContextManager::allocate(dev, ws_size));
        ws = dlprim::Tensor(cl::Buffer((cl_mem)ws_ptr.get(), true), 0,
                            dlprim::Shape(ws_size), dlprim::uint8_data);
    }
    return ws;
}
