...
// {"schema": "aten::nll_loss_forward.output(Tensor self, Tensor target, Tensor? weight,
// int reduction, int ignore_index, *, Tensor(a!) output, Tensor(b!) total_weight) ->
// (Tensor(a!), Tensor(b!))", "dispatch": "True", "default": "False"}
::std::tuple<Tensor &, Tensor &>
nll_loss_forward_out(const Tensor &self, const Tensor &target,
                     const c10::optional<Tensor> &weight, int64_t reduction,
...
// {"schema": "aten::nll_loss_backward.grad_input(Tensor grad_output, Tensor self,
// Tensor target, Tensor? weight, int reduction, int ignore_index, Tensor total_weight,
...
Tensor &nll_loss_backward_out(const Tensor &grad_output, const Tensor &self,
                              const Tensor &target, const c10::optional<Tensor> &weight,
...
// {"schema": "aten::binary_cross_entropy(Tensor self, Tensor target, Tensor?
// weight=None, int reduction=Mean) -> Tensor", "dispatch": "True", "default": "False"}
Tensor binary_cross_entropy(const Tensor &self, const Tensor &target,
                            const c10::optional<Tensor> &weight, int64_t reduction) {
    GUARD;
    TORCH_CHECK(!weight || weight->numel() == 0,
                "Weight in binar_cross_entroy isn't supported");
    Tensor self_c = self.contiguous();
    Tensor target_c = target.contiguous();
    dlprim::Tensor x = todp(self_c);
    ...
    auto op = dlprim::core::PointwiseOperationBroadcastReduce::create(
        ctx, {x.specs(), y.specs()}, {loss.specs()}, 0, dlprim::float_data,
...
// {"schema": "aten::binary_cross_entropy_backward.grad_input(Tensor grad_output, Tensor
// self, Tensor target, Tensor? weight=None, int reduction=Mean, *, Tensor(a!)
// grad_input) -> Tensor(a!)", "dispatch": "True", "default": "False"}
Tensor &binary_cross_entropy_backward_out(const Tensor &grad_output, const Tensor &self,
                                          const Tensor &target,
...
// {"schema": "aten::binary_cross_entropy_backward(Tensor grad_output, Tensor self,
// Tensor target, Tensor? weight=None, int reduction=Mean) -> Tensor", "dispatch":
// "True", "default": "False"}
Tensor binary_cross_entropy_backward(const Tensor &grad_output, const Tensor &self,
                                     const Tensor &target,
                                     const c10::optional<Tensor> &weight,
                                     int64_t reduction) {
    GUARD;
    ...
    binary_cross_entropy_backward_out(grad_output, self_c, target, weight, reduction,
                                      input_grad);
    return input_grad;
}

static Tensor &impl_softmax_out(const Tensor &self, int64_t dim, bool is_log,
                                Tensor &out) {
    GUARD;
    Tensor self_c = self.contiguous();
    dlprim::Tensor x = todp(self_c);
    dlprim::Tensor y = todp(out);
    TORCH_CHECK(dim == -1 || (0 <= dim && dim < int(x.shape().size())),
...
// {"schema": "aten::_log_softmax.out(Tensor self, int dim, bool half_to_float, *,
// Tensor(a!) out) -> Tensor(a!)", "dispatch": "True", "default": "False"}
Tensor &_log_softmax_out(const Tensor &self, int64_t dim, bool /*half_to_float*/,
                         Tensor &out) {
    return impl_softmax_out(self, dim, true, out);
}

// {"schema": "aten::_softmax.out(Tensor self, int dim, bool half_to_float, *,
// Tensor(a!) out) -> Tensor(a!)", "dispatch": "True", "default": "False"}
Tensor &_softmax_out(const Tensor &self, int64_t dim, bool /*half_to_float*/,
                     Tensor &out) {
    return impl_softmax_out(self, dim, false, out);
}

Tensor &impl_softmax_backward_data_out(const Tensor &grad_output, const Tensor &output,
                                       int64_t dim, bool is_log, Tensor &out) {
    GUARD;
    ...
    dlprim::Tensor dy = todp(grad_output_c);
    TORCH_CHECK(dim == -1 || (0 <= dim && dim < int(y.shape().size())),
                "Invalid value of dim");
    if (y.shape().size() != 2) {
        if (dim == -1)
            dim = y.shape().size() - 1;
    ...
    dlprim::core::softmax_backward(dx, y, dy, is_log, 0.0f,
                                   getExecutionContext(grad_output));
    sync_if_needed(grad_output.device());
    return out;
}

// {"schema": "aten::_log_softmax_backward_data.out(Tensor grad_output, Tensor output,
// int dim, ScalarType input_dtype, *, Tensor(a!) out) -> Tensor(a!)", "dispatch":
// "True", "default": "False"}
Tensor &_log_softmax_backward_data_out(const Tensor &grad_output, const Tensor &output,
                                       int64_t dim, ScalarType /*input_dtype*/,
                                       Tensor &out) {
    return impl_softmax_backward_data_out(grad_output, output, dim, true, out);
}

// {"schema": "aten::_softmax_backward_data.out(Tensor grad_output, Tensor output, int
// dim, ScalarType input_dtype, *, Tensor(a!) grad_input) -> Tensor(a!)", "dispatch":
// "True", "default": "False"}
Tensor &_softmax_backward_data_out(const Tensor &grad_output, const Tensor &output,
                                   int64_t dim, ScalarType /*input_dtype*/,
                                   Tensor &grad_input) {
    return impl_softmax_backward_data_out(grad_output, output, dim, false, grad_input);
}

// {"schema": "aten::mse_loss(Tensor self, Tensor target, int reduction=Mean) ->
// Tensor", "dispatch": "True", "default"
Tensor mse_loss(const Tensor &self, const Tensor &target, int64_t reduction) {
    ...
    dlprim::Context ctx(q);
    auto op = dlprim::core::PointwiseOperationBroadcastReduce::create(
        ctx, {x.specs(), lbl.specs()}, {y.specs()}, 0, x.dtype(),
        "y0 = (x0-x1)*(x0-x1);", "reduce_y0 = 0;", "reduce_y0 += y0;");
    WSGuard wsg(op->workspace(), self.device());
...
// {"schema": "aten::mse_loss_backward(Tensor grad_output, Tensor self, Tensor target,
// int reduction)
