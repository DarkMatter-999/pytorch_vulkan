...
class OCLDevImpl : public c10::impl::DeviceGuardImplInterface {
  public:
    ...
    virtual bool queryStream(const Stream & /*stream*/) const { return false; }
...
thread_local Stream OCLDevImpl::s_ =
    Stream(c10::Stream::UNSAFE, Device(OpenCLDeviceType, 0), 0);

class OCLAllocator : public at::Allocator {
  public:
    at::Device current_device() const { return ocl_impl_instance.getDevice(); }
    ...
    virtual void copy_data(void *dest, const void *src,
        ...
        q.queue().enqueueCopyBuffer(buf_src, buf_dst, 0, 0, count, q.events(),
                                    q.event("copy_data"));
...
struct _Reg {
    ...
    _Reg() { at::RegisterPrivateUse1HooksInterface(&interface); }
} reg_impl;
