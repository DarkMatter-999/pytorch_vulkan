...
std::size_t checked_bytes(const at::Tensor &tensor, const char *name) {
    ...
    TORCH_CHECK(bytes <= std::numeric_limits<VkDeviceSize>::max(), "Vulkan ", name,
                " byte count does not fit VkDeviceSize");
...
void validate_output_metadata(const at::Tensor &out,
...
void resize_output(at::Tensor &out, at::IntArrayRef sizes, const char *name) {
    ...
    const bool preserves_layout =
        out.numel() == requested_numel && out.sizes().equals(sizes);
...
void validate_exact_alias(const at::Tensor &self, const at::Tensor &out,
                          const char *name) {
    TORCH_CHECK(
        self.device() == out.device() && self.scalar_type() == out.scalar_type() &&
...
at::MemOverlapStatus classify_overlap(const at::Tensor &input, const at::Tensor &out,
                                      const char *name) {
...
const VulkanPlatform &validate_allocations(const at::Tensor *first,
    ...
    TORCH_CHECK(&platform == &out_platform &&
                    platform.device() == out_platform.device(),
                "Vulkan ", name,
    ...
    if (second) {
        ...
        TORCH_CHECK(&platform == &second_platform &&
                        platform.device() == second_platform.device(),
                    "Vulkan ", name,
                    " requires all tensors to use the same Vulkan platform/device");
    }
    return platform;
...
at::Tensor &dispatch_unary_out(const at::Tensor &input, at::Tensor &out,
    ...
    const VulkanPlatform &platform = validate_allocations(
        &input, nullptr, out, static_cast<std::size_t>(input_layout.byte_range), name);
    ...
    if (classify_overlap(input, out, name) == at::MemOverlapStatus::Full) {
        platform.compute().unary_alias(input_buffer, input_layout, output_buffer,
                                       output_layout, static_cast<uint32_t>(operation));
    } else {
        platform.compute().unary(input_buffer, input_layout, output_buffer,
                                 output_layout, static_cast<uint32_t>(operation));
    }
    return out;
}
...
at::Tensor &dispatch_tensor_tensor_out(const at::Tensor &lhs, const at::Tensor &rhs,
    ...
    if (lhs_wrapped_number || rhs_wrapped_number) {
        ...
        if (lhs_wrapped_number) {
            return dispatch_tensor_scalar_out(rhs, lhs.item(), alpha, out, operation,
                                              name, true);
        }
    ...
    if (lhs_before_overlap == at::MemOverlapStatus::Full ||
        ...
        platform.compute().tensor_tensor_alias(
            lhs_buffer, lhs_layout, rhs_buffer, rhs_layout, output_buffer,
            output_layout, static_cast<uint32_t>(operation),
...
at::Tensor &dispatch_tensor_scalar_out(const at::Tensor &tensor,
    ...
    const VulkanPlatform &platform =
        validate_allocations(&tensor, nullptr, out,
                             static_cast<std::size_t>(tensor_layout.byte_range), name);
    ...
    if (classify_overlap(tensor, out, name) == at::MemOverlapStatus::Full) {
        if (scalar_left) {
            platform.compute().scalar_tensor_alias(value, tensor_buffer, tensor_layout,
                                                   output_buffer, output_layout,
                                                   static_cast<uint32_t>(operation));
        } else {
            platform.compute().tensor_scalar_alias(tensor_buffer, tensor_layout,
                                                   output_buffer, output_layout, value,
                                                   static_cast<uint32_t>(operation));
        }
    } else if (scalar_left) {
        platform.compute().scalar_tensor(value, tensor_buffer, tensor_layout,
                                         output_buffer, output_layout,
                                         static_cast<uint32_t>(operation));
    } else {
        platform.compute().tensor_scalar(tensor_buffer, tensor_layout, output_buffer,
                                         output_layout, value,
                                         static_cast<uint32_t>(operation));
    }
    return out;
...
at::Tensor &dispatch_tensor_scalar_alias(at::Tensor &self, const at::Scalar &scalar,
    ...
    const VulkanPlatform &platform = validate_allocations(
        &self, nullptr, self, static_cast<std::size_t>(layout.byte_range), name);
...
at::Tensor &dispatch_fill_impl(at::Tensor &self, const at::Scalar &value,
                               const char *name) {
    const auto layout = validate_alias_input(self, name);
