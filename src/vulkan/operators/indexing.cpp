...
namespace {
at::Tensor argmax_tensor_impl(const at::Tensor &input, c10::optional<int64_t> dim,
                              bool keepdim, at::Tensor *provided_output) {
    TORCH_CHECK(input.device().type() == c10::DeviceType::PrivateUse1 &&
                    input.device().index() == 0,
                "Vulkan argmax requires Vulkan device index 0");
    TORCH_CHECK(input.layout() == at::kStrided,
                "Vulkan argmax requires a strided tensor");
    validate_index_input_dtype(input.scalar_type(), "argmax");
    TORCH_CHECK(input.dim() > 0,
                "Vulkan argmax does not support zero-dimensional inputs");
    ...
    TORCH_CHECK(reduce_dim >= 0 && reduce_dim < input.dim(),
                "Vulkan argmax dimension out of range");
    TORCH_CHECK(input.size(reduce_dim) > 0,
                "Vulkan argmax cannot reduce an empty dimension");
    std::vector<int64_t> shape;
    for (int64_t d = 0; d < input.dim(); ++d)
        if (keepdim || d != reduce_dim)
            shape.push_back(d == reduce_dim ? 1 : input.size(d));
    std::unique_ptr<VulkanIndexOutputAllocationGuard> index_guard;
    if (!provided_output)
        index_guard = std::make_unique<VulkanIndexOutputAllocationGuard>();
    at::Tensor output = provided_output
                            ? *provided_output
    ...
    for (int64_t d = 0; d < input.dim(); ++d) {
        TORCH_CHECK(input.size(d) <= std::numeric_limits<uint32_t>::max(),
                    "Vulkan argmax dimension is too large");
    ...
    TORCH_CHECK(&platform == &allocation_platform(out_data),
                "Vulkan argmax requires one Vulkan platform");
    platform.compute().argmax(allocation_buffer(in_data).buffer(), input_layout,
                              allocation_buffer(out_data).buffer(), output_layout,
                              static_cast<uint32_t>(reduce_dim),
                              static_cast<uint32_t>(input.size(reduce_dim)),
...
at::Tensor argmax_tensor(const at::Tensor &input, c10::optional<int64_t> dim,
                         bool keepdim) {
    return argmax_tensor_impl(input, dim, keepdim, nullptr);
}

at::Tensor &argmax_out(const at::Tensor &input, c10::optional<int64_t> dim,
                       bool keepdim, at::Tensor &out) {
    TORCH_CHECK(input.device() == out.device(),
                "Vulkan argmax out requires the same device");
    ...
    TORCH_CHECK(out.layout() == at::kStrided && out.is_contiguous() &&
                ...
                "Vulkan argmax out requires contiguous strided output with zero "
                "storage offset");
    validate_operator_rank(input.dim(), "argmax");
    ...
    if (dim) {
        ...
        for (int64_t i = 0; i < input.dim(); ++i)
            if (keepdim || i != d)
                expected.push_back(i == d ? 1 : input.size(i));
    }
    TORCH_CHECK(!dim || out.sizes().equals(expected),
                "Vulkan argmax out has the wrong shape");
    if (!dim) {
        std::vector<int64_t> all_ones(input.dim(), 1);
        TORCH_CHECK(out.sizes().equals(all_ones),
                    "Vulkan argmax out has the wrong shape");
    }
