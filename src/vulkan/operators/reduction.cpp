...
at::Tensor dispatch(const at::Tensor &, c10::OptionalArrayRef<int64_t>, bool, uint32_t, const char *, at::Tensor * = nullptr);
...
at::Tensor dispatch(const at::Tensor &input, c10::OptionalArrayRef<int64_t> dims, bool keepdim,
                    uint32_t operation, const char *name, at::Tensor *provided_output) {
    ...
    pytorch_vulkan::validate_reduction_dtype(input.scalar_type(), operation == 1u, name);
    ...
    if (reduce_numel == 0) {
        ...
        TORCH_CHECK(operation == 0u || operation == 1u || operation == 4u,
                    "Vulkan ", name, " cannot reduce an empty dimension");
        const uint32_t pattern = operation == 1u ? 0x7fc00000U : operation == 4u ? 0x3f800000U : 0U;
    ...
    platform.compute().reduction(pytorch_vulkan::allocation_buffer(in_data).buffer(), input_layout,
                                 ...
                                 mask, static_cast<uint32_t>(reduce_numel),
                                  static_cast<uint32_t>(output_numel), operation);
    return output;
}

at::Tensor reduction_backward_dispatch(const at::Tensor &input, const at::Tensor &forward,
    ...
    TORCH_CHECK(dim >= 0 && dim < input.dim(), "Vulkan reduction backward dimension out of range");
    TORCH_CHECK(input.size(dim) > 0, "Vulkan reduction backward rejects empty dimensions");
    auto grad_input = at::empty_like(input);
    auto input_layout = pytorch_vulkan::inspect_vulkan_tensor_layout(input, "reduction backward input");
    auto forward_layout = pytorch_vulkan::inspect_vulkan_tensor_layout(forward, "reduction backward output");
    auto grad_layout = pytorch_vulkan::inspect_vulkan_tensor_layout(grad_output, "reduction backward grad");
    auto result_layout = pytorch_vulkan::inspect_vulkan_tensor_layout(grad_input, "reduction backward result");
    ...
    pytorch_vulkan::validate_allocation(input_data, input_layout.allocation_bytes, "reduction backward");
    pytorch_vulkan::validate_allocation(forward_data, forward_layout.allocation_bytes, "reduction backward");
    pytorch_vulkan::validate_allocation(grad_data, grad_layout.allocation_bytes, "reduction backward");
    pytorch_vulkan::validate_allocation(result_data, result_layout.allocation_bytes, "reduction backward");
    ...
    platform.compute().reduction_backward(
        ...
        &pytorch_vulkan::allocation_buffer(result_data), result_layout,
        1u << dim, static_cast<uint32_t>(input.size(dim)), static_cast<uint32_t>(dim),
        operation, keepdim);
    return grad_input;
}

template <uint32_t Operation>
class ExtendedReductionAutograd final : public torch::autograd::Function<ExtendedReductionAutograd<Operation>> {
  public:
    static at::Tensor forward(torch::autograd::AutogradContext *ctx, const at::Tensor &input,
                              c10::OptionalArrayRef<int64_t> dims, bool keepdim) {
        at::AutoDispatchBelowAutograd guard;
        auto normalized = normalized_dims(input, dims, "reduction");
        TORCH_CHECK(normalized.size() == 1, "Vulkan reduction backward supports one dimension");
        auto output = dispatch(input, dims, keepdim, Operation, Operation == 2 ? "amax" : Operation == 3 ? "amin" : "prod");
    ...
    static torch::autograd::variable_list backward(torch::autograd::AutogradContext *ctx,
        ...
        return {reduction_backward_dispatch(saved[0], saved[1], grads[0],
                                            ...
                                            Operation == 4 ? 2u : Operation == 2 ? 0u : 1u,
...
class SoftmaxAutograd final : public torch::autograd::Function<SoftmaxAutograd> {
  public:
    static at::Tensor forward(torch::autograd::AutogradContext *ctx, const at::Tensor &input,
                              int64_t dim, bool half_to_float, bool logarithmic) {
        at::AutoDispatchBelowAutograd guard;
        auto output = logarithmic ? pytorch_vulkan::log_softmax_tensor(input, dim, half_to_float)
    ...
    static torch::autograd::variable_list backward(torch::autograd::AutogradContext *ctx,
        ...
        return {reduction_backward_dispatch(saved[0], saved[1], grads[0],
                                            ctx->saved_data["dim"].toInt(),
                                            ctx->saved_data["logarithmic"].toBool() ? 4u : 3u,
                                            true), at::Tensor(), at::Tensor(), at::Tensor()};
    }
};
}
...
at::Tensor amax_tensor(const at::Tensor &input, c10::ArrayRef<int64_t> dims, bool keepdim) {
    return dispatch(input, dims, keepdim, 2u, "amax");
}
at::Tensor amin_tensor(const at::Tensor &input, c10::ArrayRef<int64_t> dims, bool keepdim) {
...
at::Tensor &amax_out(const at::Tensor &input, c10::ArrayRef<int64_t> dims, bool keepdim,
                     at::Tensor &out) {
    dispatch(input, dims, keepdim, 2u, "amax", &out); return out;
}
at::Tensor &amin_out(const at::Tensor &input, c10::ArrayRef<int64_t> dims, bool keepdim,
                     at::Tensor &out) {
    dispatch(input, dims, keepdim, 3u, "amin", &out); return out;
}
at::Tensor prod_tensor(const at::Tensor &input, c10::OptionalArrayRef<int64_t> dims, bool keepdim,
                       c10::optional<at::ScalarType> dtype) {
    TORCH_CHECK(!dtype || *dtype == at::kFloat, "Vulkan prod supports only float32 output");
...
at::Tensor softmax_tensor(const at::Tensor &input, int64_t dim, bool half_to_float) {
    ...
    uint64_t count = 1;
    for (auto size : input.sizes()) count = checked_product(count, static_cast<uint64_t>(size), "softmax");
    uint64_t reduce = static_cast<uint64_t>(input.size(dims[0]));
    TORCH_CHECK(count <= std::numeric_limits<uint32_t>::max() && reduce <= std::numeric_limits<uint32_t>::max(),
                "Vulkan softmax exceeds dispatch limits");
    allocation_platform(in_data).compute().reduction(
        allocation_buffer(in_data).buffer(), in_layout, allocation_buffer(out_data).buffer(), out_layout,
        0u, static_cast<uint32_t>(reduce), static_cast<uint32_t>(count),
...
at::Tensor log_softmax_tensor(const at::Tensor &input, int64_t dim, bool half_to_float) {
    TORCH_CHECK(!half_to_float, "Vulkan log_softmax does not support half_to_float");
    // Keep the same validation and dispatch contract as softmax; the shader selects log mode.
    ...
    TORCH_CHECK(dim >= -input.dim() && dim < input.dim(), "Vulkan log_softmax dimension out of range");
    if (dim < 0) dim += input.dim();
    TORCH_CHECK(input.size(dim) > 0, "Vulkan log_softmax requires a non-empty dimension");
    auto output = at::empty_like(input);
    if (input.numel() == 0) return output;
    ...
    TORCH_CHECK(count <= std::numeric_limits<uint32_t>::max(), "Vulkan log_softmax exceeds dispatch limits");
    allocation_platform(in_data).compute().reduction(
        allocation_buffer(in_data).buffer(), in_layout, allocation_buffer(out_data).buffer(), out_layout,
        0u, static_cast<uint32_t>(input.size(dim)), static_cast<uint32_t>(count), 6u,
...
at::Tensor &log_softmax_out(const at::Tensor &input, int64_t dim, bool half_to_float,
    ...
    TORCH_CHECK(out.device() == input.device() && out.scalar_type() == at::kFloat &&
                    out.layout() == at::kStrided && out.is_contiguous() &&
                    out.sizes().equals(input.sizes()),
                "Vulkan log_softmax out requires a contiguous float32 output with the input shape");
...
at::Tensor &sum_out(const at::Tensor &input, c10::OptionalArrayRef<int64_t> dims, bool keepdim,
    ...
    dispatch(input, dims, keepdim, 0u, "sum", &out); return out;
}
at::Tensor &mean_out(const at::Tensor &input, c10::OptionalArrayRef<int64_t> dims, bool keepdim,
    ...
    dispatch(input, dims, keepdim, 1u, "mean", &out); return out;
}
...
at::Tensor autograd_amax(const at::Tensor &input, c10::ArrayRef<int64_t> dims, bool keepdim) {
    return ExtendedReductionAutograd<2>::apply(input, c10::OptionalArrayRef<int64_t>(dims), keepdim);
}
at::Tensor autograd_amin(const at::Tensor &input, c10::ArrayRef<int64_t> dims, bool keepdim) {
    return ExtendedReductionAutograd<3>::apply(input, c10::OptionalArrayRef<int64_t>(dims), keepdim);
}
at::Tensor autograd_prod_dim(const at::Tensor &input, int64_t dim, bool keepdim,
                             c10::optional<at::ScalarType> dtype) {
    TORCH_CHECK(!dtype || *dtype == at::kFloat, "Vulkan prod supports only float32 output");
    return ExtendedReductionAutograd<4>::apply(input, c10::ArrayRef<int64_t>(&dim, 1), keepdim);
}
...
at::Tensor autograd_log_softmax(const at::Tensor &input, int64_t dim, bool half_to_float) {
    return SoftmaxAutograd::apply(input, dim, half_to_float, true);
}

at::Tensor &softmax_backward_out(const at::Tensor &grad_output, const at::Tensor &output,
                                 int64_t dim, c10::ScalarType input_dtype, at::Tensor &grad_input) {
    TORCH_CHECK(input_dtype == at::kFloat, "Vulkan softmax backward supports only float32");
    auto result = reduction_backward_dispatch(output, output, grad_output, dim, 3u, true);
    grad_input.copy_(result);
    return grad_input;
}
at::Tensor &log_softmax_backward_out(const at::Tensor &grad_output, const at::Tensor &output,
                                     int64_t dim, c10::ScalarType input_dtype, at::Tensor &out) {
    TORCH_CHECK(input_dtype == at::kFloat, "Vulkan log_softmax backward supports only float32");
    auto result = reduction_backward_dispatch(output, output, grad_output, dim, 4u, true);
    out.copy_(result);
    return out;
}
}
