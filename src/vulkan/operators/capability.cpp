...
bool formatter_double_supported() { return platform()->supports_formatter_double(); }

void validate_vulkan_dtype(c10::ScalarType dtype, const char *operation) {
    if (dtype == at::kHalf) {
        TORCH_CHECK(
            false, "Vulkan float16 support is deferred; ", operation,
    ...
    if (dtype == at::kLong)
        return;
    TORCH_CHECK(dtype == at::kFloat || dtype == at::kBool, "Vulkan ", operation,
...
void validate_float_or_approved_bool(c10::ScalarType dtype,
...
void validate_binary_dtypes(c10::ScalarType lhs, c10::ScalarType rhs,
    ...
    TORCH_CHECK(lhs == rhs, "Vulkan ", operation_name, " requires matching dtypes");
