...
// {"schema": "aten::native_batch_norm(Tensor input, Tensor? weight, Tensor? bias,
// Tensor? running_mean, Tensor? running_var, bool training, float momentum, float eps)
// -> (Tensor, Tensor, Tensor)", "dispatch": "True", "default": "False"}
::std::tuple<Tensor, Tensor, Tensor>
native_batch_norm(const Tensor &input, const c10::optional<Tensor> &weight,
                  const c10::optional<Tensor> &bias,
                  const c10::optional<Tensor> &running_mean,
                  const c10::optional<Tensor> &running_var, bool training,
    ...
    TORCH_CHECK(weight_present == bias_present,
                "Can have affince or not affine but not partial")
    ...
    if (training) {
        ...
        bn->enqueue_update_running_stats(momentum, (1.0f - momentum), calc_mean, mean,
...
// {"schema": "aten::native_batch_norm_backward(Tensor grad_out, Tensor input, Tensor?
// weight, Tensor? running_mean, Tensor? running_var, Tensor? save_mean, Tensor?
// save_invstd, bool train, float eps, bool[3] output_mask) -> (Tensor, Tensor,
// Tensor)", "dispatch": "True", "default": "False"}
::std::tuple<Tensor, Tensor, Tensor> native_batch_norm_backward(
    const Tensor &grad_out, const Tensor &input, const c10::optional<Tensor> &weight,
    const c10::optional<Tensor> &running_mean, const c10::optional<Tensor> &running_var,
    const c10::optional<Tensor> &save_mean, const c10::optional<Tensor> &save_var,
    ...
    if (affine) {
        bn->enqueue_backward_affine(train, X, dY, mean, var, W,
                                    (bwd_data ? &dX : nullptr), 0.0,
                                    (bwd_gamma ? &dG : nullptr), 0.0,
                                    (bwd_beta ? &dB : nullptr), 0.0, eps, ws, q);
    } else {
        bn->enqueue_backward_direct(train, X, dY, mean, var, dX, 0.0, eps, ws, q);
    }
    sync_if_needed(input.device());
    return std::tuple<torch::Tensor, torch::Tensor, torch::Tensor>(x_diff, gamma_diff,
                                                                   beta_diff);
}

// {"schema": "aten::native_layer_norm(Tensor input, SymInt[] normalized_shape, Tensor?
// weight, Tensor? bias, float eps) -> (Tensor, Tensor, Tensor)", "dispatch": "True",
// "default": "True"}
std::tuple<Tensor, Tensor, Tensor>
native_layer_norm(const Tensor &input, c10::SymIntArrayRef normalized_shape,
                  const c10::optional<Tensor> &weight,
                  const c10::optional<Tensor> &bias, double eps) {
    int N = 1;
...
// {"schema": "aten::native_layer_norm_backward(Tensor grad_out, Tensor input, SymInt[]
// normalized_shape, Tensor mean, Tensor rstd, Tensor? weight, Tensor? bias, bool[3]
// output_mask) -> (Tensor, Tensor, Tensor)", "dispatch": "True", "default": "False"}
std::tuple<Tensor, Tensor, Tensor> native_layer_norm_backward(
    const Tensor &grad_out, const Tensor &input, c10::SymIntArrayRef normalized_shape,
    const Tensor &save_mean, const Tensor &save_rstd,
    const c10::optional<Tensor> &weight, const c10::optional<Tensor> &bias,
    ...
    if (bwd_gamma) {
        ...
        auto op = dlprim::core::PointwiseOperationBroadcastReduce::create(
            ctx, {X.specs(), mean.specs(), rstd.specs(), dY.specs()}, {dG.specs()}, 0,
            dlprim::float_data, "y0=(x0 - x1)*x2*x3;", "reduce_y0 = 0;",
            "reduce_y0 += y0;");
    ...
    if (bwd_beta) {
        ...
        auto op = dlprim::core::PointwiseOperationBroadcastReduce::create(
            ctx, {dY.specs()}, {dB.specs()}, 0, dlprim::float_data, "y0=x0;",
