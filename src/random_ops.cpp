...
SeqState get_random_seq(c10::Device const &d, int64_t items,
                        c10::optional<Generator> generator) {
    dlprim::RandomState &state = CLContextManager::instance().rng_state(d.index());
    size_t rounds =
        (items + dlprim::philox::result_items - 1) / dlprim::philox::result_items;
    SeqState s;
    s.seed = state.seed();
...
// {"schema": "aten::bernoulli_.float(Tensor(a!) self, float p=0.5, *, Generator?
...
Tensor &bernoulli_(Tensor &self, double p, c10::optional<Generator> generator) {
    ...
    dlprim::core::fill_random(rnd, seq.seed, seq.sequence, dlprim::core::rnd_bernoulli,
                              p, 0, getExecutionContext(self));
    sync_if_needed(self.device());
    return self;
}

// {"schema": "aten::normal_(Tensor(a!) self, float mean=0, float std=1, *, Generator?
// generator=None) -> Tensor(a!)", "dispatch": "True", "default": "False"}
Tensor &normal_(Tensor &self, double mean, double std,
    ...
    dlprim::Tensor rnd = todp(self);
    auto seq = get_random_seq(self.device(), rnd.shape().total_size(), generator);
    dlprim::core::fill_random(rnd, seq.seed, seq.sequence, dlprim::core::rnd_normal,
                              mean, std * std, getExecutionContext(self));
    sync_if_needed(self.device());
    return self;
}

// {"schema": "aten::uniform_(Tensor(a!) self, float from=0, float to=1, *, Generator?
// generator=None) -> Tensor(a!)", "dispatch": "True", "default": "False"}
Tensor &uniform_(Tensor &self, double from, double to,
    ...
    dlprim::Tensor rnd = todp(self);
    auto seq = get_random_seq(self.device(), rnd.shape().total_size(), generator);
    dlprim::core::fill_random(rnd, seq.seed, seq.sequence, dlprim::core::rnd_uniform,
                              from, to, getExecutionContext(self));
    sync_if_needed(self.device());
    return self;
}
// {"schema": "aten::native_dropout(Tensor input, float p, bool? train) -> (Tensor,
...
::std::tuple<Tensor, Tensor> native_dropout(const Tensor &input, double p,
    ...
    if (train && *train && p > 0) {
        ...
        dlprim::core::pointwise_operation({X, M}, {Y}, {1 / (1 - p)}, "y0 = x0*x1*w0;",
...
// {"schema": "aten::native_dropout_backward(Tensor grad_output, Tensor mask, float
// scale) -> Tensor", "dispatch": "True", "default": "False"}
Tensor native_dropout_backward(const Tensor &grad_output, const Tensor &mask,
    ...
    dlprim::core::pointwise_operation({dy, m}, {dx}, {scale}, "y0 = x0*x1*w0;",
