...
torch::Tensor allocate_empty(torch::IntArrayRef size, c10::optional<ScalarType> dtype,
                             c10::optional<Layout> layout, c10::optional<Device> device,
                             c10::optional<bool> /*pin_memory*/,
                             c10::optional<MemoryFormat> /*memory_format*/) {
    GUARD;
    TORCH_CHECK(!layout || *layout == Layout::Strided,
                "pytorch_ocl supports only strided layout")
    ...
    if (st == c10::kDouble && !CLContextManager::fp64(dev.index())) {
        st = c10::kFloat;
        TORCH_WARN("This device ocl:" + std::to_string(dev.index()) +
                   " does not support cl_khr_fp64, falling back to float");
    }
    return ptdlprim::new_ocl_tensor(size, dev, st);
}

torch::Tensor _reshape_alias(const Tensor &self, c10::IntArrayRef size,
...
/// "aten::empty_strided"
Tensor empty_strided(torch::IntArrayRef size, torch::IntArrayRef stride,
                     c10::optional<ScalarType> dtype, c10::optional<Layout> layout,
...
static Tensor make_contiguous_as_target_type(Tensor const &self, Tensor const &dst) {
    ...
    if (self.dtype() != dst.dtype() || !self.is_contiguous()) {
        TensorOptions options =
            TensorOptions().dtype(dst.dtype()).memory_format(MemoryFormat::Contiguous);
...
Tensor _copy_from(const Tensor &self, const Tensor &dst, bool non_blocking) {
    ...
    if (dst.device().type() == c10::DeviceType::CPU &&
        self.device().type() == OpenCLDeviceType) {
        ...
        if (dst.is_contiguous()) {
            ...
            TensorOptions options =
                TensorOptions().memory_format(MemoryFormat::Contiguous);
            ...
            dst.copy_(dst_c);
               ...
               dst.device().type() == OpenCLDeviceType) {
        Tensor c_src = make_contiguous_as_target_type(self, dst);
    ...
    } else if (self.device().type() == OpenCLDeviceType &&
               dst.device().type() == OpenCLDeviceType) {
        if (self.is_contiguous() && dst.is_contiguous()) {
            dlprim::core::pointwise_operation_broadcast(
                {todp(self)}, {todp(dst)}, {}, "y0=x0;",
            ...
            dlprim::Shape shape =
                dlprim::Shape::from_range(src_sizes.begin(), src_sizes.end());
            dlprim::Shape src_std =
                dlprim::Shape::from_range(src_stride.begin(), src_stride.end());
            dlprim::Shape tgt_std =
                dlprim::Shape::from_range(tgt_stride.begin(), tgt_stride.end());
            dlprim::core::copy_strided(shape, buffer_from_tensor(self), src_offset,
                                       src_std, buffer_from_tensor(dst), tgt_offset,
                                       tgt_std, todp(self.dtype()), todp(dst.dtype()),
                                       getExecutionContext(self.device()));
        }
        if (non_blocking)
...
// {"schema": "aten::_copy_from_and_resize(Tensor self, Tensor dst) -> Tensor",
...
// { "schema": "aten::_has_compatible_shallow_copy_type(Tensor self, Tensor from) ->
// bool"}
bool _has_compatible_shallow_copy_type(const at::Tensor &self, const at::Tensor &from) {
    ...
    return (self_keyset == from_keyset) ||
           (is_dense(self_keyset) && is_dense(from_keyset));
}
...
// {"schema": "aten::_local_scalar_dense(Tensor self) -> Scalar", "dispatch": "True",
// "default": "False"}
...
// {"schema": "aten::masked_select(Tensor self, Tensor mask) -> Tensor", "dispatch":
// "True", "default": "False"}
Tensor masked_select(const Tensor &self, const Tensor &mask) {
    ...
    TORCH_CHECK(x.shape() == m.shape(),
                "Broadasting is not implemented in masked_select yet");
    auto ec = getExecutionContext(self);
    x.to_host(ec);
    m.to_host(ec);
    size_t N = 0;
...
// {"schema": "aten::set_.source_Storage_storage_offset(Tensor(a!) self, Storage source,
// SymInt storage_offset, SymInt[] size, SymInt[] stride=[]) -> Tensor(a!)", "dispatch":
// "True", "default": "False"}
Tensor &set_source_storage_offset(Tensor &self, Storage source,
...
// {"schema": "aten::set_.source_Storage(Tensor(a!) self, Storage source) ->
...
// {"schema": "aten::resize_(Tensor(a!) self, SymInt[] size, *, MemoryFormat?
// memory_format=None) -> Tensor(a!)", "dispatch": "True", "default": "False"}
const Tensor &resize_(const Tensor &self, c10::SymIntArrayRef size,
    ...
    if (new_size >= storage_size && new_size > 0) {
        ...
        if (storage_size > 0) {
            ...
            auto q = getExecutionContext(self);
            q.queue().enqueueCopyBuffer(src, dst, 0, 0, storage_size, q.events(),
                                        q.event("copy_buffer"));
        }
...
void fallback(const c10::OperatorHandle &op, torch::jit::Stack *stack) {
    TORCH_WARN(
        "The operator '", op.schema().operator_name(), "' is not currently ",
...
TORCH_LIBRARY_IMPL(aten, PrivateUse1, m) {
    ...
    m.impl("aten::set_.source_Storage_storage_offset",
           &ptdlprim::set_source_storage_offset);
...
TORCH_LIBRARY_IMPL(aten, AutogradPrivateUse1, m) {
    m.impl("aten::_has_compatible_shallow_copy_type",
           &ptdlprim::_has_compatible_shallow_copy_type);
}
